...
import type { RemoteDirectoryEntry, RemoteDirectoryListing, RemoteServerConfig } from '../../../types/remote';
            ...
            return (
                <div className="directory-selector__list">
                    <div className="directory-selector__header">
                        ...
                        <span className="directory-selector__title">{t('chat.selectServer', 'Select Server')}</span>
                    </div>
                    {serversLoading ? (
                        <div className="directory-selector__loading">{t('common.loading', 'Loading...')}</div>
                    ) : servers.length === 0 ? (
                        <div className="directory-selector__empty">{t('chat.noServers', 'No servers found in ~/.ssh/config')}</div>
                    ) : (
                        servers.map((server) => (
                            <div
                                key={server.id}
                ...
                <>
                    ...
                    <div className="directory-selector__list">
                        {isLoading ? (
                            ...
                            <div className="directory-selector__empty" style={{ color: 'var(--color-error)' }}>{error}</div>
                        ) : directoryEntries.length === 0 ? (
                            <div className="directory-selector__empty">{t('chat.emptyDirectory', 'Empty directory')}</div>
                        ) : (
    ...
    return (
        <div className={cn('directory-selector', className)}>
            ...
            >
                ...
                {!cwdLocked && <ChevronDownIcon size={12} className={cn("transition-transform", isOpen && "rotate-180")} />}
            </Button>

            {isOpen && (
                <>
                    <div className="directory-selector__backdrop" onClick={() => setIsOpen(false)} />
                    <div className="directory-selector__dropdown">
                        {renderContent()}
                    </div>
                </>
            )}
