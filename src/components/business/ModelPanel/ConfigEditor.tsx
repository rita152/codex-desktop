...
type MonacoContentSizeChangeEvent = {
...
type MonacoEditorInstance = {
  ...
  onDidContentSizeChange: (
    listener: (event: MonacoContentSizeChangeEvent) => void
  ) => { dispose?: () => void } | void;
};
...
export function ConfigEditor({ filename, language }: ConfigEditorProps) {
  ...
  const handleEditorDidMount = (editor: MonacoEditorInstance) => {
    ...
    editor.onDidContentSizeChange((event) => {
      setEditorHeight(Math.min(Math.max(event.contentHeight, 100), 600));
    });
  ...
  return (
    <div style={glassStyle}>
      <div style={headerStyle}>
        ...
        <Button
          ...
          style={{
            ...
            fontSize: '12px',
            backgroundColor:
              editorTheme === 'vs-dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.05)',
      ...
      {error && (
        <div style={{ color: 'var(--status-error)', marginBottom: '8px', fontSize: '12px' }}>
          {error}
        </div>
      )}
